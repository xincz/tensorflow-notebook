...
def norm(x, keepdims=False):
    """
    :param x: matrix of shape (n1, n2, ..., nk)
    :param keepdims: whether keep dims or not
...
def distance(x1, x2=None, pair=True, dist_type="euclidean2"):
    """
    :param x1:
    :param x2: if x2 is None, distance between x1 and x1 will be returned
    :param pair:  if True, for i, j, x1_i, x2_j will be calculated;
                  if False, for i, x1_i, x2_i will be calculated, and it
                  requires the dimension of x1 and x2 is same
    :param dist_type: can be euclidean2, normed_euclidean2, inner_product, cosine
    :return:
    """
    if x2 is None:
        x2 = x1
...
if __name__ == "__main__":
    ...
    with tf.compat.v1.Session() as sess:
        ...
        my_assert(distance(x1, x2, pair=True, dist_type="euclidean2").eval(),
                  np.array([[9., 9., 9.],
                           [5., 5., 17.],
                           [5., 17., 5.],
                           [17., 5., 5.]]))
        my_assert(distance(x1, x2, pair=True, dist_type="normed_euclidean2").eval(),
        ...
        my_assert(distance(x1, x1[::-1], pair=False, dist_type="normed_euclidean2").eval(),
                  np.array([0.36700684, 1., 1., 0.36700684]))
        my_assert(distance(x1, x1[::-1], pair=False, dist_type="cosine").eval(),
                  np.array([0.09175171, 0.25, 0.25, 0.09175171]))
